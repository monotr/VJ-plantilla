...
bool D3DClass::Initialize(int screenWidth, int screenHeight,bool vsync, HWND hwnd, bool fullscreen,
	...
	//Now fill the display modes and find the one that matches the screen width and height
	//When a match iis found store the enumerator and denominator of the refreser rat for that monitor
	...
	//Get info on the device (the name of the video card )
	WCHAR* Description_v;
	Description_v = adapterDesc.Description;
...
A unique value that identifies the adapter. See LUID for a definition of the structure. LUID is defined in dxgi.h.
	*/
	...
	//Set regular 32-bits surface for the back buffer
	...
	//Set the feature level to DirectX 11 -We can modify this if we are planning on using multi-versions or planning on working on lower end hardware
	...
	//Create the renderer target view with the back buffer pointer
	...
	//Create an orhographic projection Matrix for 2D rendering
