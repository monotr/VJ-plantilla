...
LRESULT CALLBACK SystemClass::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam){
	...
	switch(umsg){
		...
		case WM_KEYDOWN:
		{
			//If a key is pressed send it to the input obkect so it can record that state
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight){
	...
	m_applicationName = L"Tutorial 2 Framework & Window";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT umessage, WPARAM wparam, LPARAM lparam){
	...
	switch(umessage){
		...
		//Check if the window is beign destroyed
		case WM_DESTROY:
		{
			PostQuitMessage(0);
			return 0;
